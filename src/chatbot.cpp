...
ChatBot::ChatBot(const ChatBot &source) {
  TRACE(L"ChatBot Copy Constructor\n");
  //TRACE(L"%x from %x\n", this, &source);
  _currentNode = source._currentNode;
  _rootNode = source._rootNode;
  _chatLogic = source._chatLogic;
  _chatLogic->SetChatbotHandle(this);
  _image = std::make_unique<wxBitmap>(*(source._image.get()));
}

ChatBot::ChatBot(ChatBot &&source) {
  TRACE(L"ChatBot Move Constructor\n");
  //TRACE(L"%x from %x\n", this, &source);
  _currentNode = source._currentNode;
  _rootNode = source._rootNode;
  _chatLogic = source._chatLogic;
...
ChatBot &ChatBot::operator=(const ChatBot &source) {
  TRACE(L"ChatBot Copy Assignment Operator\n");
  //TRACE(L"%x from %x\n", this, &source);
  if (&source == this)
    return *this;
  _currentNode = source._currentNode;
...
ChatBot &ChatBot::operator=(ChatBot &&source) {
 TRACE(L"ChatBot Move Assignment Operator\n");
 //TRACE(L"%x from %x\n", this, &source);
  if (&source == this)
    return *this;
  _currentNode = source._currentNode;
