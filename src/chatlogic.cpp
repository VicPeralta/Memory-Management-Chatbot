...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename) {
  ...
  //std::unique_ptr<ChatBot> temp = std::make_unique<ChatBot>(localChatBot);
  //std::unique_ptr<ChatBot> temp = std::make_unique<ChatBot>(std::move(localChatBot));
  //_chatBot = temp.get();
  ////Move semantics are used correctly with ChatBot.
  rootNode->MoveChatbotHere(std::move(localChatBot));
  /// ChatLogic has no ownership relation to the ChatBot instance and thus is no longer responsible 
